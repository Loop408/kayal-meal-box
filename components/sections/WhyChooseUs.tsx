...
export function WhyChooseUs() {
  return (
    <section id="features" className="section-padding bg-[#FFFBF0] overflow-hidden relative">
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-12 items-center mb-16">
          <div className="lg:col-span-2">
            <AnimateIn>
              <h2 className="text-4xl md:text-[45px] font-extrabold text-[#0B0C10] tracking-tight leading-tight mb-6">
          ...
          <div className="flex flex-row sm:flex-col lg:flex-row gap-4 justify-start lg:justify-end items-center w-full">
            <AnimateIn className="w-full sm:w-auto">
              <a
                ...
                className="inline-flex items-center justify-center w-full sm:w-48 px-6 py-4 rounded-full bg-[#FFC503] text-[#0B0C10] font-extrabold text-sm tracking-wide shadow-md hover:shadow-lg active:scale-95 transition-all text-center"
              >
                Subscribe Now
              </a>
            </AnimateIn>
            <AnimateIn className="w-full sm:w-auto" delay={0.1}>
              <a
                href="#menu"
                className="inline-flex items-center justify-center w-full sm:w-48 px-6 py-4 rounded-full bg-[#0B0C10] text-white font-extrabold text-sm tracking-wide shadow-md hover:shadow-lg active:scale-95 transition-all text-center"
              >
                View Menu
              </a>
            </AnimateIn>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 pt-6">
          {cardItems.map((item, idx) => (
            <AnimateIn key={idx} delay={idx * 0.08}>
              <motion.div
                className={`relative bg-white border border-[#0B0C10]/5 rounded-[22px] p-5 shadow-md transition-all duration-300 ${item.rotation} h-full flex flex-col justify-between`}
              ...
              >
                ...
                <div className="border border-[#FFC503]/50 rounded-[18px] p-4 flex flex-col items-center text-center h-full">
                  {/* Icon Wrapper */}
                  ...
                  {/* Title */}
                  <h3 className="font-extrabold text-[#0B0C10] text-[15px] leading-snug mb-3">
                    {item.title}
                  </h3>
